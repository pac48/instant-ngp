...
def parse_args():
    ...
    parser.add_argument("--zmq_port", help="Zero MQ port number for Matlab")
    parser.add_argument("--zmq_topic", help="Zero MQ topic for Matlab")
...
if __name__ == "__main__":
    ...
    if args.test_transforms:
        ...
        with tqdm(list(enumerate(test_transforms["frames"])), unit="images", desc=f"Rendering test frame") as t:
            ...
            server = ZMQServer(int(args.zmq_port), args.zmq_topic)
            while True:
                # tmp = np.matrix(frame["transform_matrix"])[:-1,:] (3, 4) [R [x;y;z]]
                arr = server.recv()
                start = time.time()
                print(start)

                transform = np.reshape(arr[0:12], (3, 4))
                w = int(arr[12])
                h = int(arr[13])
                fov_x = int(arr[14])
                testbed.fov = fov_x
                ...
                testbed.set_nerf_camera_matrix(transform)

                # w = int(test_transforms['w'])
                # h = int(test_transforms['h'])
                testbed.render_mode = ngp.RenderMode.Shade
                image = testbed.render(w, h, spp, True)
                testbed.render_mode = ngp.RenderMode.Depth
                depth = testbed.render(w, h, spp, True)
                server.send(np.concatenate((image, depth), axis=2))
