...
namespace ngp {
    ...
    void
    Testbed::Nerf::Training::set_image(int frame_idx, pybind11::array_t<float> img, pybind11::array_t<float> depth_img,
                                       float depth_scale) {
        ...
        dataset.set_training_image(frame_idx, {(int) img_buf.shape[1], (int) img_buf.shape[0]},
                                   (const void *) img_buf.ptr, (const float *) depth_buf.ptr, depth_scale, false,
    ...
    void Testbed::override_sdf_training_data(py::array_t<float> points, py::array_t<float> distances) {
        ...
        if (points_buf.ndim != 2 || distances_buf.ndim != 1 || points_buf.shape[0] != distances_buf.shape[0] ||
        ...
        for (size_t i = 0; i < points_cpu.size(); ++i) {
            ...
            pos = (pos - m_raw_aabb.min) / m_sdf.mesh_scale +
                  0.5f * (vec3(1.0f) - (m_raw_aabb.max - m_raw_aabb.min) / m_sdf.mesh_scale);
        ...
        CUDA_CHECK_THROW(cudaMemcpyAsync(m_sdf.training.positions.data(), points_cpu.data(),
                                         points_buf.shape[0] * points_buf.shape[1] * sizeof(float),
                                         cudaMemcpyHostToDevice, m_stream.get()));
        CUDA_CHECK_THROW(cudaMemcpyAsync(m_sdf.training.distances.data(), distances_cpu.data(),
                                         distances_buf.shape[0] * sizeof(float), cudaMemcpyHostToDevice,
                                         m_stream.get()));
    ...
    pybind11::dict Testbed::compute_marching_cubes_mesh(ivec3 res3d, BoundingBox aabb, float thresh) {
        ...
        CUDA_CHECK_THROW(
                cudaMemcpy(cpuverts.request().ptr, m_mesh.verts.data(), m_mesh.verts.size() * 3 * sizeof(float),
                           cudaMemcpyDeviceToHost));
        CUDA_CHECK_THROW(cudaMemcpy(cpunormals.request().ptr, m_mesh.vert_normals.data(),
                                    m_mesh.vert_normals.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost));
        CUDA_CHECK_THROW(cudaMemcpy(cpucolors.request().ptr, m_mesh.vert_colors.data(),
                                    m_mesh.vert_colors.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost));
        CUDA_CHECK_THROW(
                cudaMemcpy(cpuindices.request().ptr, m_mesh.indices.data(), m_mesh.indices.size() * sizeof(int),
                           cudaMemcpyDeviceToHost));
        ...
        return py::dict(py::arg("V") = cpuverts, py::arg("N") = cpunormals, py::arg("C") = cpucolors,
                        py::arg("F") = cpuindices);
    }

    py::array_t<float>
    Testbed::render_to_cpu(int width, int height, int spp, bool linear, float start_time, float end_time, float fps,
    ...
    py::array_t<float> Testbed::view(bool linear, size_t view_idx) const {
        ...
        CUDA_CHECK_THROW(
                cudaMemcpy2DFromArray(data, res.x * sizeof(float) * 4, render_buffer.surface_provider().array(), 0, 0,
                                      res.x * sizeof(float) * 4, res.y, cudaMemcpyDeviceToHost));
    ...
    std::vector<float> Testbed::sample_nerf(const std::vector<float> &points) {
//            ivec3 res3d;
//            vec3 ray_dir;
//            ray_dir.x = 1;
//            res3d.x = 128;
//            res3d.y = 128;
//            res3d.z = 128;
//            GPUMemory<vec4> rgba = get_rgba_on_grid(res3d, ray_dir, true, 0.01f, false);
//            std::vector<float> ret_new(rgba.size() * 4);
//            cudaMemcpy(ret_new.data(), rgba.data(), rgba.size() * 4 * sizeof(float), cudaMemcpyDeviceToHost);
//            return ret_new;

        uint32_t coord_size = sizeof(NerfCoordinate) / sizeof(float);
        if ((points.size() % coord_size) != 0) {
            throw std::runtime_error("points must be divisible by " + std::to_string(coord_size));
        }
        uint32_t num_points = points.size() / coord_size;
        GPUMatrix<float> input_coords{coord_size, num_points};
        GPUMatrix<float> output{4, num_points};
        cudaMemcpy(input_coords.data(), points.data(), num_points * coord_size * sizeof(float), cudaMemcpyHostToDevice);
        m_network->inference(m_stream.get(), input_coords, output);
        linear_kernel(compute_nerf_rgba_kernel, 0, m_stream.get(), num_points, (vec4*)output.data(), m_nerf.rgb_activation, m_nerf.density_activation, 0.01f, false);

        std::vector<float> ret(num_points * 4);
        cudaMemcpy(ret.data(), output.data(), num_points * 4 * sizeof(float), cudaMemcpyDeviceToHost);
        return ret;
    }
    ...
    PYBIND11_MODULE(pyngp, m) {
        ...
        testbed
                ...
                .def("create_empty_nerf_dataset", &Testbed::create_empty_nerf_dataset,
                     "Allocate memory for a nerf dataset with a given size", py::arg("n_images"),
                     py::arg("aabb_scale") = 1, py::arg("is_hdr") = false)
                .def("load_training_data", &Testbed::load_training_data, py::call_guard<py::gil_scoped_release>(),
                ...
                .def("init_vr", &Testbed::init_vr,
                     "Init rendering to a connected and active VR headset. Requires a window to have been previously created via `init_window`.")
                .def("view", &Testbed::view, "Outputs the currently displayed image by a given view (0 by default).",
                     py::arg("linear") = true, py::arg("view") = 0)
#ifdef NGP_GUI
                ...
                .def("screenshot", &Testbed::screenshot, "Takes a screenshot of the current window contents.",
                     py::arg("linear") = true, py::arg("front_buffer") = true)
                ...
                .def("frame", &Testbed::frame, py::call_guard<py::gil_scoped_release>(),
                ...
                .def("sample_nerf", &Testbed::sample_nerf, "sample nerf at specified points",
                     py::arg("points")
                )
                .def("train", &Testbed::train, py::call_guard<py::gil_scoped_release>(),
                     "Perform a single training step with a specified batch size.")
                ...
                .def("reload_network_from_file", &Testbed::reload_network_from_file, py::arg("path") = "",
                ...
                .def("save_snapshot", &Testbed::save_snapshot, py::arg("path"),
                     py::arg("include_optimizer_state") = false, py::arg("compress") = true,
                ...
                .def("load_file", &Testbed::load_file, py::arg("path"),
                ...
                .def_property_readonly("loss",
                                       [](py::object &obj) { return obj.cast<Testbed &>().m_loss_scalar.val(); })
                                  ...
                                  if (value && !obj.cast<Testbed &>().m_dlss_provider) {
                                      if (obj.cast<Testbed &>().m_render_window) {
                                          ...
                                          throw std::runtime_error{
                                                  "DLSS requires a Window to be initialized via `init_window`."};
                                      }
                                  }
                ...
                .def_property("root_dir",
                              ...
                              [](const py::object &obj, const std::string &value) {
                                  obj.cast<Testbed &>().set_root_dir(value);
                              }
                );
        ...
        nerf
                ...
                .def_readwrite("rendering_extra_dims_from_training_view",
                ...
                .def("find_closest_training_view", &Testbed::Nerf::find_closest_training_view,
                     "Obtain the training view that is closest to the current camera.")
                .def("set_rendering_extra_dims_from_training_view",
                ...
                .def("set_rendering_extra_dims", &Testbed::Nerf::set_rendering_extra_dims,
                     "Set the extra dims that are used for rendering.")
                .def("get_rendering_extra_dims", &Testbed::Nerf::get_rendering_extra_dims_cpu,
                     "Get the extra dims that are currently used for rendering.");
        ...
        py::class_<Testbed::Nerf::Training>(nerf, "Training")
                ...
                .def("get_extra_dims", &Testbed::Nerf::Training::get_extra_dims_cpu,
                ...
                .def("get_camera_extrinsics", &Testbed::Nerf::Training::get_camera_extrinsics, py::arg("frame_idx"),
